...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'add') {
    ...
    if (empty($etudiant_id) || empty($cours_id)) {
        ...
        $verify_query = "SELECT id FROM cours WHERE id = ? AND enseignant_id = ?";
        $verify_stmt = $conn->prepare($verify_query);
        $verify_stmt->bind_param("ii", $cours_id, $enseignant_id);
        $verify_stmt->execute();
        if ($verify_stmt->get_result()->fetch_assoc()) {
            ...
            $error = "Vous ne pouvez inscrire les étudiants que dans vos propres cours!";
        }
    }
}
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $verify_query = "SELECT i.id FROM inscriptions i 
                     JOIN cours c ON i.cours_id = c.id 
                     WHERE i.id = ? AND c.enseignant_id = ?";
    $verify_stmt = $conn->prepare($verify_query);
    $verify_stmt->bind_param("ii", $id, $enseignant_id);
    $verify_stmt->execute();
    if ($verify_stmt->get_result()->fetch_assoc()) {
        ...
        $error = "Vous ne pouvez supprimer que les inscriptions à vos propres cours!";
    }
    header("Location: inscriptions_enseignant.php");
}
...
$query_cours = "SELECT id, nom FROM cours WHERE enseignant_id = ? ORDER BY nom";
...
<head>
    ...
    <title>Inscrire Étudiants - Gestion Faculté</title>
        ...
        <div class="row">
            <div class="col-md-3">
                <div class="list-group">
                    ...
                    <a href="inscriptions_enseignant.php" class="list-group-item list-group-item-action active">Inscrire Étudiants</a>
                </div>
            </div>
            ...
            <div class="col-md-9">
                ...
                <div class="card mb-4">
                    <div class="card-header bg-primary text-white">
                        <h4>Inscrire un Étudiant à Mes Cours</h4>
                    </div>
                    <div class="card-body">
                        <?php if (empty($cours_list)): ?>
                            <div class="alert alert-warning">Vous n'avez pas encore de cours. Contactez l'administrateur.</div>
                        <?php else: ?>
                            <form method="POST">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <?php if (empty($inscriptions)): ?>
                            <div class="alert alert-info">Aucune inscription pour vos cours.</div>
                        <?php else: ?>
                                ...
                                <tbody>
                                    <?php foreach ($inscriptions as $insc): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="inscriptions_enseignant.php?delete=<?php echo $insc['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr?');">Supprimer</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
