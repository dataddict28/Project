...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'add') {
    ...
    if (empty($etudiant_id) || empty($cours_id) || empty($date_absence)) {
        ...
        // Récupérer l'ID de l'étudiant depuis l'utilisateur
        $query_etud = "SELECT id FROM etudiants WHERE user_id = ?";
        ...
        if ($etud_result) {
            $etud_id = $etud_result['id'];
            $query = "INSERT INTO absences (etudiant_id, cours_id, date_absence, statut) VALUES (?, ?, ?, ?)";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("iiss", $etud_id, $cours_id, $date_absence, $statut);
...
$etudiants_query = "SELECT u.id, u.nom FROM users u WHERE u.role = 'etudiant'";
